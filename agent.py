...
class RNDNetwork:
    ...
    def train(self, transition):
        state = torch.from_numpy(
            transition["state"][:, -self._config.embedding_train_period:].copy()).to(torch.float32).to(self._device)
...
class ActionPredictionNetwork:
    ...
    def train(self, transition):
        state = torch.from_numpy(
            transition["state"][:, -self._config.embedding_train_period - 1:-1].copy()).to(torch.float32).to(self._device)
        next_state = torch.from_numpy(
            transition["state"][:, -self._config.embedding_train_period:].copy()).to(torch.float32).to(self._device)
        action = torch.from_numpy(
            transition["action"][:, -self._config.embedding_train_period - 1:-1].copy()).to(torch.int64).to(self._device)
