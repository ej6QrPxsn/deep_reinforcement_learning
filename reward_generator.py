...
class RewardGenerator():
  ...
  def get_intrinsic_reward(self, ids, rnd_loss, embedding_output):
    ...
    clipping_factor = np.empty(len(ids))
    clipping_factor[:] = self.clipping_factor
    min_factor = np.where(max_alpha > clipping_factor, clipping_factor, max_alpha)

    intrinsic_rewards = rewards * min_factor
