...
def inference_loop(actor_indexes, infer_net, predict_net, embedding_net, transition_queue, shared_actor_datas, device, config: Config):
  ...
  select_action_output: SelectActionOutput = infer_net.select_actions(
    to_agent_input(agent_input_data, device),
    beta_table[batched_actor_output.meta_index], batch_size)
...
def train_loop(rank, infer_net, predict_net, embedding_net, sample_queue, priority_queue, device, config: Config):
  ...
  agent.online_net.load_state_dict(infer_net.state_dict())
  agent.target_net.load_state_dict(infer_net.state_dict())
  ...
  while True:
    ...
    if rank == 0:
      ...
      infer_net.load_state_dict(agent.online_net.state_dict())
